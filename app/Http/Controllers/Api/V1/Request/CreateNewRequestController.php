...
//outstation Rides outstationRides
    public function outstationRides(Request $request)
    {

    $user = auth()->user();

    $request_detail =  Request::where('user_id', $user->id)->where('is_completed', false)->where('is_cancelled', false)->where('is_out_station', true)->get();  
